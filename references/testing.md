...
- **T1 — 聚焦验证**：单元/组件测试、类型检查、编译、lint、静态分析、窄范围回归、配置解析。
- **T2 — 集成验证**：API + 持久化、消息/job、前后端、浏览器、Android 设备/模拟器、认证授权、migration、容器或服务边界。
- **T3 — 全量 / 验收**：明确要求全量测试、发布前验收/release gate，或高风险大范围修改时，执行项目定义的完整相关测试以及必要 UAT / 发布检查。

只执行 UAT 只能证明验收场景通过，不能自动代表所有 T3 回归层都已通过。计划中的检查、代码推断和历史报告都不能冒充本次通过证据。
